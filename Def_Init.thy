theory Def_Init imports Vars Com begin
...
| Assign: "vars a \<subseteq> A \<Longrightarrow>
           D A (x ::= a) (insert x A)"
| Seq:    "\<lbrakk> D A\<^sub>1 c\<^sub>1 A\<^sub>2;  D A\<^sub>2 c\<^sub>2 A\<^sub>3 \<rbrakk> \<Longrightarrow>
           D A\<^sub>1 (c\<^sub>1;; c\<^sub>2) A\<^sub>3"
...
| While:  "\<lbrakk> vars b \<subseteq> A; D A c A' \<rbrakk> \<Longrightarrow>
           D A (WHILE b DO c) A"
