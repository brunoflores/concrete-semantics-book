...
lemma apreservation:
  ...
  (* By induction on the typing derivation \<Gamma> \<turnstile> \<alpha> : \<tau> *)
...
lemma aprogress:
  "\<Gamma> \<turnstile> a : \<tau> \<Longrightarrow> \<Gamma> \<turnstile> s \<Longrightarrow> \<exists>v. taval a s v"
(* By induction on the typing derivation \<Gamma> \<turnstile> \<alpha> : \<tau> *)
...
(* Progress for boolean expressions *)
lemma bprogress:
  "\<Gamma> \<turnstile> b \<Longrightarrow> \<Gamma> \<turnstile> s \<Longrightarrow> \<exists>v. tbval b s v"
(* By rule induction on the typing derivation *)
