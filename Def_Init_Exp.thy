theory Def_Init_Exp imports Vars begin

(* Initialization-sensitive expressions evaluation. *)

(* We take None as the uninitialized value. *)
...
| "bval (Not b) s =
    (case bval b s of
     ...
     | Some bv \<Rightarrow> Some(\<not> bv))"
| "bval (And b\<^sub>1 b\<^sub>2) s =
    (case (bval b\<^sub>1 s, bval b\<^sub>2 s) of
      (Some bv\<^sub>1, Some bv\<^sub>2) \<Rightarrow> Some (bv\<^sub>1 & bv\<^sub>2)
     | _ \<Rightarrow> None)"
