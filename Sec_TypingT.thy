theory Sec_TypingT imports Sec_Type_Expr begin

(* A Syntax Directed System *)

(* This a modified WHILE rule with one that does not admit anything
   higher than level 0 in the condition. Key idea: WHILE conditions
   must not depend on confidential data. If they don't, then termination
...
| Assign: "\<lbrakk> sec x \<ge> sec a;  sec x \<ge> l \<rbrakk> \<Longrightarrow>
           l \<turnstile> x ::= a"
| Seq:    "\<lbrakk> l \<turnstile> c\<^sub>1; l \<turnstile> c\<^sub>2 \<rbrakk> \<Longrightarrow>
           l \<turnstile> c\<^sub>1;; c\<^sub>2"
| If:     "\<lbrakk> max (sec b) l \<turnstile> c\<^sub>1;  max (sec b) l \<turnstile> c\<^sub>2 \<rbrakk> \<Longrightarrow>
           l \<turnstile> IF b THEN c\<^sub>1 ELSE c\<^sub>2"
| While:  "\<lbrakk> sec b = 0; 0 \<turnstile> c \<rbrakk> \<Longrightarrow>
...
(* New fact: any program that is type correct, but not at level 0,
   must terminate. If the program is type correct at some level,
   but not at level 0, it does not contain WHILE loops. *)
lemma termi_if_non0: "\<lbrakk> l \<turnstile> c; l \<noteq> 0 \<rbrakk> \<Longrightarrow> \<exists> t. (c, s) \<Rightarrow> t"
...
| Seq':       "\<lbrakk> l \<turnstile>' c\<^sub>1; l \<turnstile>' c\<^sub>2 \<rbrakk> \<Longrightarrow>
               l \<turnstile>' c\<^sub>1;; c\<^sub>2"
| If':        "\<lbrakk> sec b \<le> l;  l \<turnstile>' c\<^sub>1;  l \<turnstile>' c\<^sub>2 \<rbrakk> \<Longrightarrow>
               l \<turnstile>' IF b THEN c\<^sub>1 ELSE c\<^sub>2"
| While':     "\<lbrakk> sec b = 0;  0 \<turnstile>' c \<rbrakk> \<Longrightarrow>
               0 \<turnstile>' WHILE b DO c"
| anti_mono': "\<lbrakk> l \<turnstile>' c;  l' \<le> l \<rbrakk> \<Longrightarrow>
               l' \<turnstile>' c"
